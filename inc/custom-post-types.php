...
function trip_kailash_render_package_meta_box( $post ) {
    ...
    ?>
    <style>
        .tk-meta-field { margin-bottom: 20px; }
        .tk-meta-field label { display: block; font-weight: 600; margin-bottom: 5px; }
        .tk-meta-field input[type="text"],
        .tk-meta-field input[type="number"],
        .tk-meta-field select { width: 100%; max-width: 400px; }
        .tk-meta-field input[type="checkbox"] { margin-right: 5px; }
        .tk-key-stops { margin-top: 10px; }
        .tk-key-stop-item { margin-bottom: 10px; }
        .tk-key-stop-item input { width: 100%; max-width: 400px; }
        .tk-add-stop { margin-top: 10px; }
    </style>

    <div class="tk-meta-field">
        <label for="trip_length"><?php esc_html_e( 'Trip Length', 'trip-kailash' ); ?></label>
        <input type="text" id="trip_length" name="trip_length" value="<?php echo esc_attr( $trip_length ); ?>" placeholder="e.g., 7 nights / 8 days">
        <p class="description"><?php esc_html_e( 'Enter the duration of the trip (e.g., "7 nights / 8 days")', 'trip-kailash' ); ?></p>
    </div>
    ...
    <div class="tk-meta-field">
        ...
        <select id="deity" name="deity">
            ...
            <option value="shiva" <?php selected( $deity, 'shiva' ); ?>><?php esc_html_e( 'Shiva', 'trip-kailash' ); ?></option>
            <option value="vishnu" <?php selected( $deity, 'vishnu' ); ?>><?php esc_html_e( 'Vishnu', 'trip-kailash' ); ?></option>
            <option value="devi" <?php selected( $deity, 'devi' ); ?>><?php esc_html_e( 'Devi', 'trip-kailash' ); ?></option>
        </select>
    </div>
    ...
    <div class="tk-meta-field">
        ...
        <select id="difficulty" name="difficulty">
            ...
            <option value="Easy" <?php selected( $difficulty, 'Easy' ); ?>><?php esc_html_e( 'Easy', 'trip-kailash' ); ?></option>
            <option value="Moderate" <?php selected( $difficulty, 'Moderate' ); ?>><?php esc_html_e( 'Moderate', 'trip-kailash' ); ?></option>
            <option value="Challenging" <?php selected( $difficulty, 'Challenging' ); ?>><?php esc_html_e( 'Challenging', 'trip-kailash' ); ?></option>
        </select>
    </div>

    <div class="tk-meta-field">
        <label for="best_months"><?php esc_html_e( 'Best Months', 'trip-kailash' ); ?></label>
        <input type="text" id="best_months" name="best_months" value="<?php echo esc_attr( $best_months ); ?>" placeholder="e.g., March-May, September-November">
        <p class="description"><?php esc_html_e( 'Enter the best months to visit', 'trip-kailash' ); ?></p>
    </div>

    <div class="tk-meta-field">
        <label for="price_from"><?php esc_html_e( 'Price From (INR)', 'trip-kailash' ); ?></label>
        <input type="number" id="price_from" name="price_from" value="<?php echo esc_attr( $price_from ); ?>" min="0" step="1">
        <p class="description"><?php esc_html_e( 'Enter the starting price in Indian Rupees', 'trip-kailash' ); ?></p>
    </div>
            ...
            if ( ! empty( $key_stops ) ) {
                foreach ( $key_stops as $index => $stop ) {
                    ...
                    <div class="tk-key-stop-item">
                        <input type="text" name="key_stops[]" value="<?php echo esc_attr( $stop ); ?>" placeholder="<?php esc_attr_e( 'Enter stop name', 'trip-kailash' ); ?>">
                    </div>
                    <?php
                }
            } else {
                ?>
                <div class="tk-key-stop-item">
                    <input type="text" name="key_stops[]" value="" placeholder="<?php esc_attr_e( 'Enter stop name', 'trip-kailash' ); ?>">
                </div>
                <?php
            }
            ?>
        </div>
        <button type="button" class="button tk-add-stop" id="tk-add-stop"><?php esc_html_e( 'Add Stop', 'trip-kailash' ); ?></button>
...
function trip_kailash_save_package_meta( $post_id ) {
    // Check nonce
    if ( ! isset( $_POST['trip_kailash_package_meta_nonce'] ) || 
...
function trip_kailash_render_guide_meta_box( $post ) {
    ...
    <style>
        .tk-meta-field { margin-bottom: 20px; }
        .tk-meta-field label { display: block; font-weight: 600; margin-bottom: 5px; }
        .tk-meta-field input[type="number"] { width: 100%; max-width: 200px; }
        .tk-meta-field textarea { width: 100%; max-width: 600px; height: 100px; }
    </style>

    <div class="tk-meta-field">
        <label for="years_of_experience"><?php esc_html_e( 'Years of Experience', 'trip-kailash' ); ?></label>
        <input type="number" id="years_of_experience" name="years_of_experience" value="<?php echo esc_attr( $years_of_experience ); ?>" min="0" step="1">
...
function trip_kailash_save_guide_meta( $post_id ) {
    // Check nonce
    if ( ! isset( $_POST['trip_kailash_guide_meta_nonce'] ) || 
...
function trip_kailash_render_lodge_meta_box( $post ) {
    ...
    <style>
        .tk-meta-field { margin-bottom: 20px; }
        .tk-meta-field label { display: block; font-weight: 600; margin-bottom: 5px; }
        .tk-meta-field input[type="text"] { width: 100%; max-width: 400px; }
        .tk-amenities { margin-top: 10px; }
        .tk-amenity-item { margin-bottom: 10px; }
        .tk-amenity-item input { width: 100%; max-width: 400px; }
        .tk-add-amenity { margin-top: 10px; }
    </style>

    <div class="tk-meta-field">
        <label for="location"><?php esc_html_e( 'Location', 'trip-kailash' ); ?></label>
        <input type="text" id="location" name="location" value="<?php echo esc_attr( $location ); ?>" placeholder="e.g., Kathmandu, Nepal">
            ...
            if ( ! empty( $amenities ) ) {
                foreach ( $amenities as $index => $amenity ) {
                    ...
                    <div class="tk-amenity-item">
                        <input type="text" name="amenities[]" value="<?php echo esc_attr( $amenity ); ?>" placeholder="<?php esc_attr_e( 'Enter amenity name', 'trip-kailash' ); ?>">
                    </div>
                    <?php
                }
            } else {
                ?>
                <div class="tk-amenity-item">
                    <input type="text" name="amenities[]" value="" placeholder="<?php esc_attr_e( 'Enter amenity name', 'trip-kailash' ); ?>">
                </div>
                <?php
            }
            ?>
        </div>
        <button type="button" class="button tk-add-amenity" id="tk-add-amenity"><?php esc_html_e( 'Add Amenity', 'trip-kailash' ); ?></button>
        <p class="description"><?php esc_html_e( 'Add amenities available at this lodge (e.g., Hot Water, Puja Room, WiFi)', 'trip-kailash' ); ?></p>
    </div>
...
function trip_kailash_save_lodge_meta( $post_id ) {
    // Check nonce
    if ( ! isset( $_POST['trip_kailash_lodge_meta_nonce'] ) || 
