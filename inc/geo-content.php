...
function tk_get_quick_facts($package = null) {
    ...
    $includes = array();
    if ($has_lodge) $includes[] = 'Lodge Accommodation';
    if ($includes_meals) $includes[] = 'All Meals';
    if ($includes_rituals) $includes[] = 'Puja & Rituals';
    if ($has_helicopter) $includes[] = 'Helicopter Service';
...
function tk_wrap_semantic_content($content) {
    ...
    // Add semantic wrapper based on post type
    $itemtype = 'Article';
    ...
    <nav class="tk-breadcrumbs" aria-label="<?php esc_attr_e('Breadcrumb', 'trip-kailash'); ?>">
        <ol class="tk-breadcrumbs__list" itemscope itemtype="https://schema.org/BreadcrumbList">
            ...
            if (is_singular('pilgrimage_package')) {
                ...
                ?>
                <li class="tk-breadcrumbs__item tk-breadcrumbs__item--current" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
                ...
                ?>
                <li class="tk-breadcrumbs__item tk-breadcrumbs__item--current" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
                    <span itemprop="name"><?php echo esc_html($term->name); ?> <?php esc_html_e('Pilgrimages', 'trip-kailash'); ?></span>
                    <meta itemprop="position" content="<?php echo $position; ?>" />
                </li>
                <?php
            } elseif (is_singular()) {
            ...
            <?php while ($related->have_posts()) : $related->the_post(); ?>
                <article class="tk-related-packages__item">
                    ...
                    <p class="tk-related-packages__meta">
                        ...
                        <?php endif; ?>
                        <?php if ($price) : ?>
                            <span class="tk-related-packages__price"><?php echo esc_html('From ₹' . number_format($price)); ?></span>
                        <?php endif; ?>
                    </p>
                </article>
            <?php endwhile; ?>
